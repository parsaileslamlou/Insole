...
>
> Every number in this document is measured on the four `_02` captures and
...
> the headline accuracies and their intervals — are in
> [`real_results.md`](real_results.md), regenerated by `scripts/train_real.py`.
> Nothing below reflects them. Read a number from here as "what one session
> showed at stage 13", never as "what the project measures now".
