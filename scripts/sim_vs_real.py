"""STAGE-13 RECORD, `_02` SET ONLY -- NOT THE CURRENT RESULTS.

Everything this script reads and prints is the four `_02` captures and
nothing else. It did not move when the `_03` session was added, and it is not
meant to: it is the stage-13 record, kept as it was measured. The current
two-session results -- 224 stances over `_02` and `_03`, leave-one-session-out,
the headline accuracies and their intervals -- are in `docs/real_results.md`,
regenerated by `scripts/train_real.py`. Nothing printed below reflects them.
In particular D2's sim-trained figures are the 113-stance `_02` ones; the
224-stance versions are `scripts/train_real.py` section 9.
