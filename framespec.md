...
Readings are raw ADC counts. Calibration to force units happens on the host (see `pipeline/calibration.py`), never in firmware, so recalibration does not require a reflash.
...
The sum runs over field *values*, not over the characters of the line. This makes it formatting-invariant: `007` and `7` contribute identically, and a stray space cannot change the result.

Firmware must accumulate in a `uint32_t`. Signed overflow is undefined behaviour in C++; unsigned wraparound is defined. Overflow of the accumulator is harmless because 2³² is an exact multiple of 256, so `(sum % 2³²) % 256` equals `sum % 256`.
...
Both were considered. Binary is ~19 bytes per frame against CSV's ~44, which at 100 Hz is 1.9 kB/s versus 4.4 kB/s. The link is USB CDC at 12 Mbit/s, so both consume well under 1% of available bandwidth. The efficiency advantage is real and buys nothing at this operating point.
...
CSV also gets framing for free. A newline byte cannot occur inside a decimal number, so `\n` is an unambiguous frame boundary. A binary sync byte offers no such guarantee: `0xAA` can appear inside a 12-bit reading (`0x0AAA` contains it), so every candidate boundary must be confirmed against the checksum. The practical consequence is resynchronization behaviour — a lost byte in a CSV stream corrupts exactly one line, and the next newline restores alignment, whereas a lost byte in a binary stream desynchronizes the parser until a sync search succeeds.
...
In this format the newline handles boundary recovery, so SYNC is not the resynchronization mechanism. It earns its place for two other reasons.

First, it rejects tail fragments. A fragment that has lost its head has no `INS` prefix and fails the sync gate, where it might otherwise present a plausible field count and parse cleanly.
...
The channel is a short wired USB link — no radio, no long cable, no burst-error environment. The realistic failure mode is a dropped or truncated frame caused by a buffer overrun or host stall, not the subtle multi-byte corruption that CRCs exist to catch. Truncation is caught by the checksum together with the field-count gate.
...
An 8-bit counter wraps every 2.56 s at 100 Hz, which makes a dropout of exactly 256 frames (or any multiple) invisible in the sequence numbers. 16 bits pushes that failure mode out to an 11-minute dropout, which the timestamp would independently flag. The cost is roughly three characters per frame.
...
- **Coincidental collision.** Two distinct frames collide on the same checksum byte roughly 1 time in 256.
...
The timestamp is generated on the board, so the pipeline is insensitive to transport jitter and to batched delivery. It is not insensitive to *loss*: dropped frames leave gaps in the time axis, which breaks anything assuming uniform sample spacing (windowing, impulse integration, loading-rate estimation). SEQ exists to make that loss detectable rather than silent; the handling policy lives in the ingestion stage.
...
Each gate catches something no other gate catches. In particular, field count and integer parse together are not sufficient: a frame truncated at the end can retain a correct field count and parse cleanly while its checksum field is missing entirely and a truncated sensor reading has been shifted into its place. Only the checksum rejects that line.
...
- Physical FSR placement to be confirmed against the channel order in §3. Exact 2D coordinates are required later for centre-of-pressure and heatmap work and will live in a single shared coordinate table.

## Appendix: board selection

The QT Py ESP32-S3 was evaluated and rejected. It breaks out four analog pins (A0–A3), of which only two (A2, A3) are on ADC1. This design requires six ADC1 channels; ADC2 is unusable when WiFi is active.
