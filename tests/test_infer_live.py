...
failure and the direct run keeps its counts. A check whose input is a
GENERATED, gitignored artifact that has not been built yet prints SKIP and
counts as neither -- see skip() -- so a fresh clone is green before
scripts/bakeoff.py has run, without any check claiming to have passed.
"""
...
from insole.representations import (
    REPRESENTATIONS, SHIPPED, features_under, gains_from_doc,
)
...
SKIPPED = []            # (name, reason) for every check a missing artifact stopped


def skip(name, reason):
    """A check that cannot run because a generated artifact is absent.

    Not a pass and not a failure: the check has no evidence either way, so
    counting it as either would be a claim about something never measured. It
    prints SKIP with the command that produces the artifact. This exists so a
    FRESH CLONE is green: data/sim/features_sessions.csv is gitignored, and
    treating its absence as a test failure made `pytest` red on a clean
    checkout until scripts/bakeoff.py had run -- a real failure and a
    not-yet-generated file reported as the same thing.
    """
...
def check_anchor_and_model():
    ...
    if os.path.exists(mpath) and os.path.exists(fpath):
        ...
        # data/sim/features_sessions.csv is generated by scripts/bakeoff.py and
        # gitignored, so it is absent in a fresh clone. Absent is not wrong --
        # there is nothing to refit against yet -- so this skips rather than
        # failing, and pytest stays green on a clean checkout.
        t.skip("model_lda.json predicts as a fresh fit_lda on features_sessions.csv",
               "models/model_lda.json (committed) or "
               "data/sim/features_sessions.csv (generated by bakeoff, "
               "gitignored) not present; run: python scripts/bakeoff.py")
    return t.result()



# ---------------------------------------------------------------------------
# 10. The model's representation drives the feature path, and cannot be faked
# ---------------------------------------------------------------------------
def check_model_representation():
    """infer_live applies the representation the MODEL names, or refuses.

    This is the test that the model file format no longer decides which
    representation the project can ship. Before, features were always computed
    under representations.SHIPPED and a model naming anything else was
    rejected, so the persisted real models had to be fitted under B for this
    script to accept them -- the loader constraining the science. Now the field
    drives the transform.

    The models used here are the committed ones with `meta.representation`
    rewritten. The coefficients then no longer match the representation, which
    does not matter: what is under test is which transform the FEATURES are
    computed under, and those are read out of the --out CSV, not the labels.
    """
    t = Tally()
    gm = C.load_gain_match(IL.DEFAULT_GAIN)
    gains = gains_from_doc(gm)
    capture = os.path.join(REAL, "walk_03.csv")

    # -- every committed model declares a representation this build has ------
    for path in sorted(glob.glob(os.path.join(MODELS, "model_*.json"))):
        meta = load_model(path).get("meta", {})
        rep = meta.get("representation")
        t(f"{os.path.basename(path)} declares a known representation",
          rep in REPRESENTATIONS, f"representation={rep!r}")

    # -- both real models exist, under both representations, correctly labelled
    for stem, want in (("model_lda_real", SHIPPED), ("model_qda_real", SHIPPED),
                       ("model_lda_real_raw", "raw"), ("model_qda_real_raw", "raw")):
        path = os.path.join(MODELS, stem + ".json")
        if not os.path.exists(path):
            t.skip(f"{stem}.json is persisted and labelled {want}",
                   "models/*_real*.json not present (generated); "
                   "run: python scripts/train_real.py")
            continue
        meta = load_model(path).get("meta", {})
        t(f"{stem}.json is labelled {want} and flags shipped correctly",
          meta.get("representation") == want
          and meta.get("is_shipped_default") == (want == SHIPPED),
          f"representation={meta.get('representation')!r} "
          f"is_shipped_default={meta.get('is_shipped_default')!r}")

    def run_with_rep(rep, extra=()):
        """infer_live on `capture` with the default model relabelled `rep`.

        The JSON is edited as text, not through load_model: load_model returns
        numpy arrays, which json.dump cannot write back.
        """
        with tempfile.TemporaryDirectory() as tmp:
            with open(os.path.join(MODELS, "model_qda_real.json")) as f:
                src = json.load(f)
            if rep is _ABSENT:
                src["meta"].pop("representation", None)
            else:
                src["meta"]["representation"] = rep
            path = os.path.join(tmp, "m.json")
            with open(path, "w") as f:
                json.dump(src, f)
            return run_infer([capture, "--model", path] + list(extra))

    # -- honoured: each of the three, features identical to the batch path ---
    per_rep = {}
    for rep in REPRESENTATIONS:
        code, out, rows = run_with_rep(rep)
        ref = features_under(pd.read_csv(capture),
                             D.merge_close(D.find_stances(
                                 pd.read_csv(capture)[D.SENSOR_COLS].sum(axis=1).to_numpy())),
                             "x", rep, gains if rep == "gain_matched" else None)
        same, detail = stream_features_match(rows, ref)
        t(f"representation {rep}: accepted, and the streamed features equal the batch "
          f"path under {rep}", code == 0 and same, f"exit={code} {detail}")
        t(f"representation {rep}: the banner names it",
          f"({rep})" in out, out.split("features  :")[-1].split("\n")[0].strip()[:90])
        per_rep[rep] = [tuple(float(r[c]) for c in FEATS) for r in rows]

    # -- and the three are genuinely different, so none of this is a no-op ---
    t("raw and conductance produce DIFFERENT features (the transform is real)",
      per_rep.get("raw") and per_rep["raw"] != per_rep.get("conductance"))
    t("conductance and gain_matched produce DIFFERENT features",
      per_rep.get("conductance") and per_rep["conductance"] != per_rep.get("gain_matched"))

    # -- refused: no silent fallback, and both sides named -------------------
    code, out, rows = run_with_rep(_ABSENT)
    t("a model with NO representation field is refused, not assumed shipped",
      code == 2 and not rows, f"exit={code} rows={len(rows)}")
    t("...and the message says the field is absent and names the refit scripts",
      "records no representation" in out and "train_real.py" in out)

    code, out, rows = run_with_rep("logspace")
    t("a model naming an unknown representation is refused",
      code == 2 and not rows, f"exit={code}")
    t("...and the message names both the model's value and this build's list",
      "'logspace'" in out and "gain_matched" in out)

    code, out, rows = run_with_rep("gain_matched", extra=["--gain", "none"])
    t("gain_matched under --gain none is refused rather than demoted to conductance",
      code == 2 and not rows, f"exit={code}")
    t("...and the message says identity gains would silently demote C to B",
      "demote" in out and "gain_matched" in out and "none" in out)

    # The same model IS accepted once the gains are available.
    code, out, rows = run_with_rep("gain_matched")
    t("the same gain_matched model is accepted when a gain document is loaded",
      code == 0 and len(rows) > 0, f"exit={code} stances={len(rows)}")
    return t.result()


_ABSENT = object()       # sentinel: drop meta.representation entirely
...
def test_model_representation():
    p, f = check_model_representation()
    assert f == 0, f"{f} check(s) failed; see the FAIL lines above"
