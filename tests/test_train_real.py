...
def check_sim_reproduction():
    ...
    if not os.path.exists(fpath):
        # Generated by scripts/bakeoff.py and gitignored, so absent in a fresh
        # clone. Absent is not wrong -- there is simply nothing to compare
        # against yet -- so this skips rather than failing.
        t.skip("representation A reproduces the sim bake-off frame and number",
               "data/sim/features_sessions.csv not present (it is generated and "
               "gitignored); run: python scripts/bakeoff.py")
        return t.result()
